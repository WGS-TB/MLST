...
for x in range(1,41): 
	...
	#run a bash sub command to sort the variants text file and return the firt k variants
        ...
        new_cmd = "cat *_1.fq > ClpX_all_"+str(x)+"_1.fa" #append all the first of the pairs together
        new_cmd2 ="cat *_2.fq > ClpX_all_"+str(x)+"_2.fa" #append all the second of the pairs together
        os.system(new_cmd) #run the command
        os.system(new_cmd2) #run the command
        os.system("rm clpX_*") #remove unneccessary files for the next iteration.
