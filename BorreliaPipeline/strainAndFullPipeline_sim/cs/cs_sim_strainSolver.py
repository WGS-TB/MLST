...
def strainSolver(dataPath, refStrains, outputPath, objectiveOption, globalILP_option='all', timelimit=600, gap=8,
    ...
    print "\n\n-_-_-_-_-_- here comes the boom -_-_-_-_-_-\n"
    ...
    A = defineProblem(numOfComb, [int(x) for x in dims])

    # Now we solve the cs problem
    strainProps, sumOfProps, error = solve_cs(A, b, numOfComb)

    strainNums = np.nonzero(strainProps)[0]
    strainNumsIndexes = [n + 1 for n in strainNums]

    output = pd.DataFrame({'ST': strainNumsIndexes})
    output = pd.concat([output, strains.iloc[strainNums]])
    output['Proportions'] = strainProps

    print "\n-_-_-_-_-_- here goes the boom -_-_-_-_-_-\n\n"
