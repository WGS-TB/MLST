...
def Simulate_strains(numIter,strainRef,sampleDir,outputDir,samplesDir,simFilesDir,sim_type,hamming_dist,Type):
	...
	print sum(proportions[0])
        ...
        for i in range(len(strain_list)):
            strain_dict[tuple(strain_list[i])] = proportions[0][i]*100
            strain_var_proportions[tuple(strain_list[i])] = Compute_Variant_proportions(strain_list[i],proportions[0][i])
...
def main():
    ...
    #run the simulation
    precision,recall,total_var_dist = Simulate_strains(args["numOfiter"],args["strainRef"],args["masterDir"]+'/'+sampleDir+"/"+sample,args["masterDir"]+'/'+outputDir,args["masterDir"]+'/'+sampleDir,args["masterDir"]+'/'+tempDir,args['Sim_Type'],args["HammingDist"],args["Mut_Rec_Type"])
